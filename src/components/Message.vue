    ...
    <div v-for="(value , index) in shares" :key="index">
      <div class="message">
        <div class="flex">
          ...
          <img src="../assets/detail.png" class="icon detail" @click="$router.push({
...
export default {
  ...
  methods:{
      ...
      if(result){
        this.shares[index].like.forEach((element) => {
          if(element.user_id == this.$store.state.user.id){
            axios({
              method:"detail",
              url:"https://tranquil-fjord-01037.herokuapp.com/api/like",
              data:{
        ...
        axios
          .post("https://tranquil-fjord-01037.herokuapp.com/api/like",{
    ...
    del(index){
      axios
        .delete(
          'https://tranquil-fjord-01037.herokuapp.com/api/shares/' + this.shares[index].item.id
          )
        .then(response => {
          console.log(response);
          this.$router.go({
    ...
    async getShares(){
      ...
      const shares = await axios.get(
        "https://tranquil-fjord-01037.herokuapp.com/api/shares/" 
      );
      for(let i = 0 ;i < shares.data.data.length; i++){
        await axios
          .get(
            "https://tranquil-fjord-01037.herokuapp.com/api/shares/" + shares.data.data[i].id
          )
          .then(response => {
